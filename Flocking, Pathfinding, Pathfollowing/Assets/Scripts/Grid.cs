            ...
            for (int row = 0; row < rows; ++row)
             {
                 for (int col = 0; col < columns; ++col)
                 {
                    ...
                    if (row >= 3 && row <=26  && col >= 3 && col <= 36)
                    {
                      
                        node.blocked = true;
                        node.gameObject.GetComponent<Collider2D>().isTrigger = false;
                    }
                    if(row == 14 && col == 19)
                   {
        ...
        private void Update()
        {
            if (Input.GetMouseButton(0))
            {
                ...
                if (row >= 0 && row < _nodes.GetLength(0) && column >= 0 && column < _nodes.GetLength(1))
                {
                    ...
                    if (pathfollowingScript.gamestate == Pathfollowing.Gamestates.DrawingTracks)
                    {
                        ...
                        if (node.blocked == true)
                        {
                            ...
                            node.terrainType = GridNode.TerrainType.Road;
                        }
                    }

                    else if (pathfollowingScript.gamestate == Pathfollowing.Gamestates.DrawingPath)
                    {
                        if (node.blocked == true)
                        {
                            node.blocked = false;
                          
                                node.myRenderer.color = Color.magenta;
