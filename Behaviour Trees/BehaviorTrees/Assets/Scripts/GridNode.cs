    ...
    public class GridNode : MonoBehaviour, IDropHandler
    {
        ...
        public bool blocked
        {
            ...
            set
            {
                ...
                if (_blocked)
                {
                    _renderer.color = GameManager._blockedColor;
                }
                else
                {
                    ...
                    {
                        //_renderer.sprite = GameManager.gameManager._fores;
                        _renderer.color = GameManager._forestColor;
                    }
                    else if (_terrainType == TerrainType.Road)
                    {
                        //_renderer.sprite = GameManager.gameManager.road;
                        _renderer.color = GameManager._roadColor;
                    }
                    else
                    {
                        // _renderer.sprite = GameManager.gameManager.water;
                        _renderer.color = GameManager._waterColor;
                    }
                }
            }
        }
        ...
        public IList<KeyValuePair<int, GridNode>> GetNeighbors(int row, int column, bool include_diagonal = false)
        {
            ...
            int end_row = Mathf.Min(row + 1, GameManager.gameManager.grid.nodes.GetLength(0) - 1);
            int end_col = Mathf.Min(column + 1, GameManager.gameManager.grid.nodes.GetLength(1) - 1);
            ...
            for (int row_index = start_row; row_index <= end_row; ++row_index)
            {
                for (int col_index = start_col; col_index <= end_col; ++col_index)
                {
                    if (include_diagonal || row_index == row || col_index == column)
                    {
                        ...
                        {
                            neighbors.Add(new KeyValuePair<int, GridNode>(GameManager.gameManager.grid.nodes[row_index, col_index].f, GameManager.gameManager.grid.nodes[row_index, col_index]));
                        }
                    }
                }
            }
        ...
        {
            if (GameManager.gameManager.gamestate == GameManager.GameStates.PlacingGameObjects)
            {
                if (!blocked && gridNodeType == GridNodeType.Normal)
                {
                    ...
                    GameManager.gameManager.grid.specialNode = new KeyValuePair<GridNodeType, GridNode>(gridNodeType, this);
